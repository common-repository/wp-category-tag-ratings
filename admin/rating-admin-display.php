...
function display_rating_functiona_own() {
	...
	if ( empty( $AOI_postratings_none_template ) ) {
		$AOI_postratings_none_template = '<<%AIO_RATING_IMAGES%>><br> ( No Ratings Yet )';
	}

	$AOI_all_ready_rate_template = get_option( 'AOI_all_ready_rate_template' );
	if ( empty( $AOI_all_ready_rate_template ) ) {
		$AOI_all_ready_rate_template = '<<%AIO_RATING_IMAGES%>><br> ( <<%AIO_TOTAL_NO_RAT%>> votes, average: <<%AIO_RATINGS_AVERAGE%>> out of <<%AIO_TOTAL_RATINGS%>> )';
	}

	$AOI_after_user_rating_template = get_option( 'AOI_after_user_rating_template' );
	if ( empty( $AOI_after_user_rating_template ) ) {
		$AOI_after_user_rating_template = '<<%AIO_RATING_IMAGES%>><br> ( <<%AIO_TOTAL_NO_RAT%>> votes, average: <<%AIO_RATINGS_AVERAGE%>> out of <<%AIO_TOTAL_RATINGS%>> )';
	}
...
function rating_option_functiona_own() {
	...
	if ( empty( $rating_img_name ) ) {
		$rating_img_name = 'star';
	}

	$google_rich = get_option( 'google_rich' );
	if ( empty( $google_rich ) ) {
		$google_rich = 'on';
	}
	...
	if ( empty( $postratings_allow_to ) ) {
		$postratings_allow_to = 3;
	}

	$postratings_user_type = get_option( 'postratings_user_type' );
	if ( empty( $postratings_user_type ) ) {
		$postratings_user_type = 2;
	}

	$rating_restric_day = get_option( 'rating_restric_day' );
	if ( empty( $rating_restric_day ) ) {
		$rating_restric_day = 1;
	}
					...
					<table class="form-table">
						<tr>
							...
							<td class="admin_rating_img">
								...
								<p class="rating-radio">
									<input type="radio" name="postratings_image" onclick="" value="star" <?php echo 'star' === $rating_img_name ? 'checked="checked"' : ''; ?>></p>
								<p>
									<img src="<?php echo esc_url( plugins_url() . '/wp-category-tag-ratings/images/rating/star/star.png' ); ?>" alt="star rating">
								</p></td>
						</tr>
						<tr>
							...
							<td class="admin_rating_img">
								<p class="rating-radio">
									<input type="radio" name="postratings_image" onclick="" value="custom" <?php echo 'custom' === $rating_img_name ? 'checked="checked"' : ''; ?>>
								</p>
								<?php
								if ( get_option( 'rating_none' ) ) {
									...
									for ( $i = 1; $i <= $total_rating; $i ++ ) {
										if ( $i <= 2 ) {
											?><img src="<?php echo esc_url( $rating_full_img ); ?>" alt="custom rating"><?php
										} else {
											?><img src="<?php echo esc_url( $rating_none_img ); ?>" alt="custom rating"><?php
										}
									}
									?></p><?php
								} else {
									?><p><img src="<?php echo esc_url( plugins_url() . '/wp-category-tag-ratings/images/rating/heart/heart.png' ); ?>" alt="custom rating"></p><?php
								}
								?>
							</td>
						</tr>
						<tr class="csutom_rating_img <?php echo ( 'custom' == $rating_img_name ) ? 'custom-active' : ''; ?>">
							<td class="h_title">No Rating Images</td>
							<td>
								<div class="custom-img-uploding-wrapper">
						<span class="example-img">Ex: 
							<img src="<?php echo esc_url( $sample_rating_none_img ); ?>" alt="custom rating">
						</span>
									<span class="custom_img_input_box">
										<input id="rating_none" name="rating_none" type="text" class="custom_image_url_upload" value="<?php echo esc_url( $rating_none_img );
										?>"></span>
								</div>
						...
						<tr class="csutom_rating_img <?php echo 'custom' === $rating_img_name ? 'custom-active' : ''; ?>">
							...
							<td>
								<div class="custom-img-uploding-wrapper">
						<span class="example-img">Ex: 
							<img src="<?php echo esc_url( $sample_rating_full_img ); ?>" alt="custom rating">
						</span>
									<span class="custom_img_input_box"><input id="rating_full" type="text" name="rating_full" class="custom_image_url_upload"
									                                          value="<?php echo $rating_full_img; ?>"></span>
								</div>
								<p class="admin_suggestion"><span id="admin_suggestion">we recommended rating image width 25px</span></p>
							</td>
						</tr>
						<tr class="csutom_rating_img <?php echo 'custom' === $rating_img_name ? 'custom-active' : ''; ?>">
							...
							<td>
								<div class="custom-img-uploding-wrapper">
						<span class="example-img">Ex: 
							<img src="<?php echo esc_url( $sample_rating_hover_img ); ?>" alt="custom rating">
						</span>
									<span class="custom_img_input_box">
										<input id="rating_hover" type="text" name="rating_hover" class="custom_image_url_upload"
										       value="<?php echo esc_url( $rating_hover_img ); ?>"></span>
								</div>
								<p class="admin_suggestion"><span id="admin_suggestion">we recommended rating image width 25px</span></p>
							</td>
						</tr>
						<tr class="csutom_rating_img <?php echo 'custom' === $rating_img_name ? 'custom-active' : ''; ?>">
							...
							<td>
								<div class="custom-img-uploding-wrapper">
						<span class="example-img">Ex:
						<img src="<?php echo esc_url( $sample_rating_avg_img ); ?>" alt="custom rating">
						</span>
									<span class="custom_img_input_box">
										<input id="rating_avg" type="text" name="rating_avg" class="custom_image_url_upload" value="<?php echo esc_url( $rating_avg_img );
										?>"></span>
								</div>
								<p class="admin_suggestion"><span id="admin_suggestion">we recommended rating image width 25px</span></p>
							</td>
						...
						<tr>
							...
							<td><input type="text" name="no_of_rating" value="<?php echo $total_rating; ?>" class="num-error-val" maxlength="5"
							           onkeyup="this.value = minmax(this.value, 3, 100);"> <span id="num-error-msg"></span></td>
						</tr>
						<tr>
							<td class="h_title">Enable Google Rich Snippets</td>
							<td>

								<input type="radio" name="google_rich" onclick="" value="on" <?php echo ( 'on' === $google_rich ) ? 'checked="checked"' : ''; ?>> ON &nbsp;&nbsp;&nbsp;
								<input type="radio" name="google_rich" onclick="" value="off" <?php echo ( 'off' === $google_rich ) ? 'checked="checked"' : ''; ?>> OFF
							</td>
						</tr>
						<tr>
							...
							<td>
								<select id="postratings_allow_to" name="postratings_allow_to" size="1">
									<option value="1" <?php echo ( 1 == $postratings_allow_to ) ? 'selected="selected"' : '' ?>>Guest Only
									</option>
									<option class="aio-rating-user" value="2" <?php echo ( 2 == $postratings_allow_to ) ? 'selected="selected"' : '' ?>>Users Only
									</option>
									<option value="3" <?php echo ( 3 == $postratings_allow_to ) ? 'selected="selected"' : ''; ?>>Users And Guest
									</option>
								</select></td>
						</tr>
						<tr>
							...
							<td>
								<div id="post_ratings_ristric_to">
									<select id="post_ratings_ristric_type" name="postratings_ristric_to" size="1">
										<option class="aio-rating-user" value="1" <?php if ( 1 == $postratings_user_type ) {
											echo 'selected="selected"';
										} ?>>User
										</option>
										<option value="2" <?php if ( 2 == $postratings_user_type ) {
											echo 'selected="selected"';
										} ?>>IP address
										</option>
										<option value="3" <?php if ( 3 == $postratings_user_type ) {
											echo 'selected="selected"';
										} ?>>days
										</option>
									</select>
								</div>
								<input type="text" value="<?php echo $rating_restric_day; ?>" <?php if ( 3 != $postratings_user_type ) {
									echo 'style="display:none"';
								} ?>" name="rating_restric_day" id="post_rating_restric_day" class="num-error-val"> <span id="num-error-msg"></span>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<div class="custom-img-uploding-wrapper">
									<span><input id="aio_rating_loader" name="aio_rating_loader" type="text" class="custom_image_url_upload"
									             value="<?php echo $aio_rating_loader; ?>"></span>
