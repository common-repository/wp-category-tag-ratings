...
function user_given_rating_to_post() {
	if ( isset( $_POST['post_id'], $_POST['user_rat'], $_POST['action'], $_POST['nonce'] ) && ! empty( $_POST['post_id'] ) && ! empty( $_POST['user_rat'] ) && ! empty( $_POST['action'] ) ) {

		// Verify nonce.
		if ( ! wp_verify_nonce( $_POST['nonce'], 'aio_postratings_' . absint( $_POST['post_id'] ) . '-nonce' ) ) {
			wp_die( 'Nonce Verification failed' );
		}

		global $wpdb, $user_identity, $user_ID;
		$master_api_return_value = '';
		$user_given_post_rat     = absint( $_POST['user_rat'] );
		...
		if ( $user_given_post_rat > 0 && $post_ID > 0 ) {
			...
			if ( ! empty( $Allow_To_Rate ) ) {
				...
				if ( ! empty( $user_Rating_Restriction ) ) {
					...
					$aio_total_rating        = ( $aio_total_rating + absint( $user_given_post_rat ) );
					...
					// Print Rating.
					sucessfully_rating( $post_ID, $aio_average_post_rating, $aio_total_rating, $aio_rating_count );
					exit();
				}
 ...
 *
 * @param $host string we have get host name on function parameter if we hav't get any parameter than we have decault get id adress
 *
 * @return string user ip address if not recevied host value but we have get host value on paramter than we have return host name
 */
	...
	if ( ! empty( $currentuser_allow_to_rate ) ) {
			...
			if ( ! empty( $avg_loop_half_rating ) ) {
				if ( $i <= $total_no_of_avg_loop ) {
					if ( $i == $avg_loop_half_rating ) {
						$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_avg_img ) . '"  alt="rating_' . $i . '" data-rat="' . $i . '" onclick="give_rate(' . absint( $ratings_post_id ) . ', ' . $i . ');" 	onmouseover="aio_rating_hover(' . absint( $ratings_post_id ) . ', ' . $i . ');" onmouseout="aio_rating_none(' . absint( $ratings_post_id ) . ', ' . $i . ');" data-img-name="avg_rating" >';

					} else {
						$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_full_img ) . '"  alt="rating_' . $i . '" data-rat="' . $i . '" onclick="give_rate(' . absint( $ratings_post_id ) . ', ' . $i . ');"  onmouseover="aio_rating_hover(' . absint( $ratings_post_id ) . ', ' . $i . ');" onmouseout="aio_rating_none(' . absint( $ratings_post_id ) . ', ' . $i . ');" data-img-name="rating_full" >';
					}
				} else {
					$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_none_img ) . '"  alt="rating_' . $i . '" data-rat="' . $i . '" onclick="give_rate(' . absint( $ratings_post_id ) . ', ' . $i . ');" 	onmouseover="aio_rating_hover(' . absint( $ratings_post_id ) . ', ' . $i . ');" onmouseout="aio_rating_none(' . absint( $ratings_post_id ) . ', ' . $i . ');" data-img-name="rating_none" >';
				}
			} else {
				if ( $i <= $total_no_of_avg_loop ) {
					$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_full_img ) . '"  alt="rating_' . $i . '" data-rat="' . $i . '" onclick="give_rate(' . absint( $ratings_post_id ) . ', ' . $i . ');"  onmouseover="aio_rating_hover(' . absint( $ratings_post_id ) . ', ' . $i . ');" onmouseout="aio_rating_none(' . absint( $ratings_post_id ) . ', ' . $i . ');"  data-img-name="rating_full">';
		...
		for ( $i = 1; $i <= $total_rating; $i ++ ) {
			...
			if ( ! empty( $avg_loop_half_rating ) ) {
				if ( $i <= $total_no_of_avg_loop ) {
					if ( $i == $avg_loop_half_rating ) {
						$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_avg_img ) . '"  alt="rating_' . $i . '" data-rat="' . $i . '" >';
					} else {
						$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_full_img ) . '"  alt="rating_' . $i . '" data-rat="' . $i . '" >';
					}
				} else {
					$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_none_img ) . '"  alt="rating_' . $i . '" data-rat="' . $i . '"  >';
				}
			} else {
				if ( $i <= $total_no_of_avg_loop ) {
					$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_full_img ) . '"  alt="rating_' . $i . '" data-rat="' . $i . '" >';
...
function aoi_post_rating_none( $ratings_post_id, $total_rating, $rating_none_img, $rating_full_img, $rating_avg_img ) {
	...
	for ( $i = 1; $i <= $total_rating; $i ++ ) {
		$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_none_img ) . '"  alt="rating_' . $i . '" data-rat="' . $i . '" onclick="give_rate(' . absint( $ratings_post_id ) . ', ' . $i . ');" onmouseover="aio_rating_hover(' . absint( $ratings_post_id ) . ', ' . $i . ');" onmouseout="aio_rating_none(' . absint( $ratings_post_id ) . ', ' . $i . ');"  >';
	}
 ...
 *
 * @param  $ratings_post_id   int Current post or tag/category id
 * @param  $total_no_of_avg   float|string rating avarage
 * @param  $aio_total_rating  string no of user voted on this post (no of vote counter)
 * @param  $aio_rating_count  integer of given user rating
 */
...
function sucessfully_rating( $ratings_post_id, $total_no_of_avg, $aio_total_rating, $aio_rating_count ) {
	...
	for ( $i = 1; $i <= $total_rating; $i ++ ) {
		...
		if ( ! empty( $avg_loop_half_rating ) ) {
			if ( $i <= $total_no_of_avg_loop ) {
				if ( $i == $avg_loop_half_rating ) {
					$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_avg_img ) . '"  alt="rating_' . $i . '" >';
				} else {
					$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_full_img ) . '"  alt="rating_' . $i . '" >';
				}
			} else {

				$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_none_img ) . '"  alt="rating_' . $i . '"  >';
			}
		} else {
			if ( $i <= $total_no_of_avg_loop ) {
				$AIO_RATING_IMAGES .= '<img id="' . absint( $ratings_post_id ) . '_' . $i . '" src="' . esc_url( $rating_full_img ) . '"  alt="rating_' . $i . '" data-rat="' . $i . '">';
